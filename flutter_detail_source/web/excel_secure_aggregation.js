...
(function () {
  ...
  function prepare(matrix) {
    ...
    const rows = matrix.slice(first).filter(Array.isArray).map(r => {
  ...
  function infos(headers) {
    return headers.map((name, i) => ({ i, name, n: normalize(name), c: compact(name) })).filter(x => x.name);
  }
  ...
  function resolve(headers, requested) {
    ...
    for (const key of Object.keys(aliases)) {
      if (wanted === key || aliases[key].includes(wanted)) {
    ...
    const scored = list.map(x => {
      ...
      let score = 0;
      for (const t of tokens) if (xt.has(t)) score += 3;
      for (const t of tokens) if (x.n.includes(t)) score += 1;
      return { ...x, score };
  ...
  function parse(query) {
    ...
    if (ranked) groupText = ranked[1].trim();
    else if (by) groupText = by[1].trim();
    if (!groupText) {
      const which = q.match(/\bwhich\s+(.+?)\s+(?:has|have|with)\b/);
      if (which) groupText = which[1].trim();
    }
    if (!groupText) {
      const what = q.match(/\bwhat\s+(.+?)\s+(?:has|have|with)\b/);
      if (what) groupText = what[1].trim();
    }
    groupText = groupText.replace(/^(the|a|an)\s+/, '').trim();

    let measureText = '';
    if (ranked) {
      measureText = ranked[2].trim().replace(/\b(?:highest|lowest|maximum|minimum)\s+/, '').replace(/\b(?:average|avg|mean|sum|total|count)\b\s*/, '').trim();
    } else if (by) {
  ...
  function aggregate(matrix, query) {
    ...
    for (const row of prepared.rows) {
      ...
      bucket.count++;
      if (measure.index >= 0) {
        const n = numeric(row[measure.index]);
    ...
    return {
      success:true,
      route:'operation',
      operation:{ action:'aggregate', group_by:[prepared.headers[group.index]], measure:measure.index >= 0 ? prepared.headers[measure.index] : null, aggregation:spec.mode, sort:spec.direction, limit:spec.limit, columns, rows, row_count:rows.length, source_mutated:false },
      message:`Calculated ${aggregateLabel} by ${prepared.headers[group.index]} locally (${rows.length} result groups).`,
      local_secure:true,
      diagnostics,
      source_mutated:false,
    };
