...
# These are ontology concepts, not column-name routing rules. Tokens provide
# corroborating semantic evidence and are combined with local value/type rules.
...
_ROLE_MAP = {
    ...
    "foreign_key_candidate": "identifier",
...
def _value_shape_evidence(series: pd.Series) -> dict[str, Any]:
    # Only aggregate statistics leave this layer. No individual values are
    # returned or persisted, which keeps diagnostics privacy-safe.
...
def _business_role(column_name: str, inferred_role: str | None) -> tuple[str | None, float, list[str]]:
    ...
    for concept, aliases in _BUSINESS_CONCEPTS.items():
        ...
        if hit:
            matches.append((concept, len(hit), hit))
    if not matches:
        return None, 0.0, []
    matches.sort(key=lambda item: item[1], reverse=True)
    concept, score, evidence = matches[0]
    base = 0.72 if score == 1 else 0.88
    if inferred_role in {"date", "datetime"} and concept.endswith("_date"):
        base += 0.08
    if inferred_role in {"currency_measure", "numeric_measure"} and concept in {"revenue", "cost", "profit", "discount"}:
        base += 0.06
...
class SemanticSchema:
    ...
    def to_safe_dict(self) -> dict[str, Any]:
        return {
            ...
            "table_grain": self.table_grain,
            "fields": [f.to_dict() for f in self.fields],
        }


class SemanticSchemaEngine:
    """Shared semantic layer built on the existing registered rule engine."""
    ...
    def infer(self, df: pd.DataFrame, dataset_id: str = "local") -> SemanticSchema:
        ...
        for name in df.columns:
            ...
            value_evidence = _value_shape_evidence(series)

            # Deterministic type/shape roles supplement, rather than replace,
            # the existing rule engine. These are generic and value-derived.
            if mapped_role is None and pd.api.types.is_bool_dtype(series):
                mapped_role = "boolean"
            elif mapped_role is None and pd.api.types.is_numeric_dtype(series):
                mapped_role = "numeric_measure"
            elif mapped_role is None and pd.api.types.is_string_dtype(series):
                if value_evidence["cardinality_ratio"] <= 0.02 and value_evidence["unique_count"] <= 50:
                    mapped_role = "categorical"
                else:
                    mapped_role = "free_text"

            if mapped_role == "date" and physical == "datetime":
                mapped_role = "datetime"
            ...
            if mapped_role in {"boolean", "numeric_measure", "datetime"} and physical == mapped_role:
                confidence = max(confidence, 0.9)

            cardinality = int(value_evidence["unique_count"])
            ...
            if winning:
                evidence_categories.append("registered_rule")
            if business:
                evidence_categories.append("semantic_tokens")
            fields.append(SemanticField(
                name=name,
                physical_type=physical,
                semantic_role=mapped_role,
                ...
                evidence={
                    "rule": winning.rule_name if winning else None,
                    "rule_evidence": winning.evidence if winning else {},
                    "shape": value_evidence,
