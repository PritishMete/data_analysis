...
def test_organizational_structure_routes_use_supabase_provider(monkeypatch):
    ...
    monkeypatch.setattr(routes,"list_sections",lambda claims,workspace_id,include_inactive,location_id:calls.append(("sections",workspace_id,location_id)) or {"sections":[]})
    ...
    assert client.get("/v1/authz/organizations/locations",params={"workspace_id":"org1","location_id":"loc1"},headers=headers).status_code==200
    ...
    assert calls==[("locations","org1"),("sections","org1","loc1"),("assignments","org1")]
