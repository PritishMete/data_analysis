...
if ([string]::IsNullOrWhiteSpace($FlutterRoot)) { $FlutterRoot = Join-Path $RepoRoot 'flutter_detail_source' }
$ExpectedBaseline = '8053000369222f897d12927b3599a5e65274b479'
$AllowedAcceptanceFiles = @('tools/run_final_windows_acceptance.ps1','tools/final_windows_acceptance.py','tools/final_windows_browser_acceptance.py','tools/start_insightflow.ps1','main.py','requirements.txt','common/dashboard.py','common/chat_reasoning.py','common/order_cleaning.py','common/customer_cleaning.py','flutter_detail_source/lib/detail_analysis_main.dart','flutter_detail_source/lib/embedded_report_scroll.dart','flutter_detail_source/lib/test_motion_mode.dart','flutter_detail_source/lib/detail_analysis_router.dart','flutter_detail_source/lib/features/dashboard/app_bar_actions.dart','flutter_detail_source/lib/widgets/system_status_button.dart','flutter_detail_source/lib/src/renderer/shaders.dart','flutter_detail_source/lib/tech_background.dart','flutter_detail_source/lib/widgets/shared/glass_effect.dart','flutter_detail_source/lib/widgets/shared/lightweight_liquid_glass.dart','flutter_detail_source/pubspec.yaml','flutter_detail_source/build_detail_analysis.ps1','flutter_detail_source/test/tech_background_test.dart','flutter_detail_source/test/embedded_report_scroll_test.dart','flutter_detail_source/test/test_motion_mode_test.dart','flutter_detail_source/test/detail_analysis_router_test.dart','tests/test_business_analysis_model_cache.py','tests/test_browser_acceptance_helpers.py','tests/test_detail_upload.py','tests/test_detail_enhancements.py','tests/test_chat_reasoning.py')
...
$DatasetPaths=@('orders_raw.csv','customers_raw.csv','products_raw.csv','regions_raw.csv')|%{Join-Path $DataRoot $_};$SourceHashesBefore=@{};foreach($path in $DatasetPaths){$SourceHashesBefore[(Split-Path $path -Leaf)]=Hash $path}
...
foreach($pair in @(@('REAL DATASET SESSION','real_dataset_session'),@('ALL-TIME VALUES','all_time'),@('NORTH','north'),@('NORTH + 2025','north_2025'),@('NORTH + 2025 + CLOTHING','north_2025_clothing'),@('PRIVACY','privacy'))){$v=$false;if($lo -and $lo.checks.PSObject.Properties.Name -contains $pair[1]){$v=[bool]$lo.checks.($pair[1])};Gate $pair[0] $v 'live backend/data adapter'}
...
$map=@{'PRODUCT CONTEXT'='product';'COMPARISON CONTEXT'='comparison';'BOOLEAN / THEN'='boolean';'SESSION SUMMARY'='summary';'META NON-MUTATION'='meta_context';'FAILED-QUERY RECOVERY'='unsupported';'CHART REGRESSION'='charts_or_schema';'STAR SCHEMA REGRESSION'='charts_or_schema';'COPY / SELECTION'='copy_or_selection';'DIAGNOSTICS UI'='diagnostics_ui';'SUGGESTED FOLLOW-UPS'='suggestion_chips';'REPORT GENERATION'='reports';'PDF VISUAL QA'='pdf_visual_qa'};foreach($k in $map.Keys){$v=$false;if($bo){$key=$map[$k];if($key -eq 'reports'){$reportItems=@($bo.reports.PSObject.Properties);$v=$reportItems.Count -eq 5 -and @($reportItems|?{$_.Value.passed}).Count -eq 5}elseif($key -eq 'product'){$v=@('product','product_profit','product_2025')|%{$bo.queries.$_.passed};$v=(@($v|?{$_}).Count -eq 3)}elseif($key -eq 'boolean'){$v=@('boolean','boolean_then','then')|%{$bo.queries.$_.passed};$v=(@($v|?{$_}).Count -eq 3)}elseif($key -eq 'summary'){$v=@('summary','global_summary')|%{$bo.queries.$_.passed};$v=(@($v|?{$_}).Count -eq 2)}elseif($key -eq 'meta_context'){$v=@('meta','meta_context')|%{$bo.queries.$_.passed};$v=(@($v|?{$_}).Count -eq 2)}elseif($bo.checks.PSObject.Properties.Name -contains $key){$v=[bool]$bo.checks.$key}elseif($bo.queries.PSObject.Properties.Name -contains $key){$v=[bool]$bo.queries.$key.passed}elseif($bo.reports.PSObject.Properties.Name -contains $key){$v=[bool]$bo.reports.$key.passed}};Gate $k $v 'automated browser acceptance'};Gate 'PRIVACY' ($R['PRIVACY'].status -eq 'PASS' -and $bo -and $bo.checks.supported_data_local) 'diagnostics evidence + browser network capture'
...
$SourceHashesAfter=@{};foreach($path in $DatasetPaths){$SourceHashesAfter[(Split-Path $path -Leaf)]=Hash $path};$hashesUnchanged=(ConvertTo-Json $SourceHashesBefore -Compress) -ceq (ConvertTo-Json $SourceHashesAfter -Compress);Gate 'SOURCE CSV HASHES UNCHANGED' $hashesUnchanged 'SHA-256 before and after complete acceptance';$R['SOURCE CSV HASHES']=[ordered]@{before=$SourceHashesBefore;after=$SourceHashesAfter}
...
$report=[ordered]@{title='INSIGHTFLOW FINAL WINDOWS ACCEPTANCE';branch=$branch;commit=$sha;baseline=$ExpectedBaseline;results=$R;files_changed=@('main.py','requirements.txt','common/dashboard.py','common/chat_reasoning.py','common/order_cleaning.py','common/customer_cleaning.py','flutter_detail_source/lib/detail_analysis_main.dart','flutter_detail_source/lib/embedded_report_scroll.dart','flutter_detail_source/lib/test_motion_mode.dart','flutter_detail_source/lib/detail_analysis_router.dart','flutter_detail_source/lib/features/dashboard/app_bar_actions.dart','flutter_detail_source/lib/widgets/system_status_button.dart','flutter_detail_source/lib/src/renderer/shaders.dart','flutter_detail_source/lib/tech_background.dart','flutter_detail_source/lib/widgets/shared/glass_effect.dart','flutter_detail_source/lib/widgets/shared/lightweight_liquid_glass.dart','flutter_detail_source/pubspec.yaml','flutter_detail_source/build_detail_analysis.ps1','flutter_detail_source/test/tech_background_test.dart','flutter_detail_source/test/embedded_report_scroll_test.dart','flutter_detail_source/test/test_motion_mode_test.dart','flutter_detail_source/test/detail_analysis_router_test.dart','tests/test_business_analysis_model_cache.py','tests/test_browser_acceptance_helpers.py','tests/test_detail_upload.py','tests/test_detail_enhancements.py','tests/test_chat_reasoning.py','tools/run_final_windows_acceptance.ps1','tools/final_windows_acceptance.py','tools/final_windows_browser_acceptance.py','tools/start_insightflow.ps1');defects=$Defects;remaining_limitations=$Limitations;user_manual_testing_required='NO';code_changed='YES';final_verdict=$verdict};$report|ConvertTo-Json -Depth 40|Set-Content $JsonPath -Encoding utf8

$lines=[System.Collections.Generic.List[string]]::new();$lines.Add('INSIGHTFLOW FINAL WINDOWS ACCEPTANCE');$lines.Add('');$lines.Add("BACKEND TESTS: $($R['BACKEND TESTS'].passed) passed, $($R['BACKEND TESTS'].failed) failed, $($R['BACKEND TESTS'].skipped) skipped, $($R['BACKEND TESTS'].warnings) warnings");$lines.Add("LIVE STUDENT RUNTIME TEST: $($R['LIVE STUDENT RUNTIME TEST'].status)");$lines.Add("FLUTTER TESTS: $($R['FLUTTER TESTS'].status)");$lines.Add("FLUTTER BUILD: $($R['FLUTTER BUILD'].status)");$lines.Add("DETAIL ANALYSIS BUILD ID: $id");foreach($x in @('BUILD ID MATCH','BACKEND DIAGNOSTICS','WINDOWS START SCRIPT','WINDOWS RESTART SCRIPT','WINDOWS HEALTH CHECK','REAL DATASET SESSION','ALL-TIME VALUES','NORTH','NORTH + 2025','NORTH + 2025 + CLOTHING','PRODUCT CONTEXT','COMPARISON CONTEXT','BOOLEAN / THEN','SESSION SUMMARY','SUGGESTED FOLLOW-UPS','META NON-MUTATION','FAILED-QUERY RECOVERY','REPORT GENERATION','PDF VISUAL QA','CHART REGRESSION','STAR SCHEMA REGRESSION','COPY / SELECTION','DIAGNOSTICS UI','PRIVACY','SOURCE CSV HASHES UNCHANGED')){$lines.Add("$($x.ToUpperInvariant()): $($R[$x].status)")};$lines.Add("BACKEND RESTARTED AFTER FINAL CHANGE: $(if($fr.code -eq 0){'YES'}else{'NO'})");$lines.Add("FINAL SMOKE AFTER RESTART: $($R['FINAL SMOKE AFTER RESTART'].status)");$lines.Add('USER MANUAL TESTING REQUIRED: NO');$lines.Add('CODE CHANGED: YES');$lines.Add('FILES CHANGED: main.py; requirements.txt; common/dashboard.py; common/chat_reasoning.py; flutter_detail_source/lib/detail_analysis_main.dart; flutter_detail_source/lib/features/dashboard/app_bar_actions.dart; flutter_detail_source/lib/widgets/system_status_button.dart; tests/test_detail_upload.py; tests/test_detail_enhancements.py; tests/test_chat_reasoning.py; tools/run_final_windows_acceptance.ps1; tools/final_windows_acceptance.py; tools/final_windows_browser_acceptance.py; tools/start_insightflow.ps1');$lines.Add('DEFECTS FOUND AND FIXED:');$lines.Add('  - Chapter 6 lacked the structured response contract and ignored active follow-up filters.');$lines.Add('  - Regional profit ranked physical region keys instead of aggregating business-region labels.');$lines.Add('  - Deterministic chat-plan constructors omitted the comparison-scope default.');$lines.Add('  - Context-inherited PDF filenames were rejected by the local report route.');$lines.Add('  - Narrow viewport dataset summary overflowed; diagnostics control was absent from standalone Detail Analysis.');$lines.Add('  - Browser acceptance now validates rendered Prompt 1, chat scroll activity, diagnostics, copy/selection, and actual PDF content.');$lines.Add('REMAINING LIMITATIONS:');$lines.Add('  - A gate is PASS only when executed against the live Windows runtime.');$lines.Add("FINAL VERDICT: $verdict");$lines|Set-Content $TxtPath -Encoding utf8;Get-Content $TxtPath|Out-Host
exit $Exit
